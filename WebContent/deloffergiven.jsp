<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%@ page import="java.io.*,java.util.*,java.sql.*"%>
<%@ page import="javax.servlet.http.*,javax.servlet.*"%>     
<!DOCTYPE html PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN" "http://www.w3.org/TR/html4/loose.dtd">
<html>
<head>
<meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
<title>Delete Offer</title>
</head>
<body>
<%
	try {	
		String userid = (String)session.getAttribute( "userid" );
		String url = "jdbc:mysql://group28.caxv2nuxlyui.us-east-1.rds.amazonaws.com:3306/db28";
		Class.forName("com.mysql.jdbc.Driver");
		
		Connection con = DriverManager.getConnection(url, "user28", "group28pass");
		
		Statement stmt = con.createStatement();
		
		String query = "SELECT * FROM offerGiven where Username = '" + userid + "'" ;
		
		out.print("<table>");

		//make a row
		out.print("<tr>");
		//make a column
		out.print("<td>");
		out.print("Passenger");
		out.print("</td>");
		
		out.print("<td>");
		out.print("Date");
		out.print("</td>");
		
		out.print("<td>");
		out.print("Time");
		out.print("</td>");
		
		out.print("<td>");
		out.print("Car");
		out.print("</td>");
		
		
		
		
		out.print("</tr>");
		
		
		ResultSet resultset = stmt.executeQuery(query);
		
		while (resultset.next()) {
			//make a row
			out.print("<tr>");
			//make a column
			out.print("<td>");
			out.print(resultset.getString("Passenger"));
			out.print("</td>");
			
			out.print("<td>");
			out.print(resultset.getString("Date"));
			out.print("</td>");
			
			
			out.print("<td>");
			out.print(resultset.getString("Time"));
			out.print("</td>");
			
			out.print("<td>");
			out.print(resultset.getString("carName"));
			out.print("</td>");
			
		
			out.print("</tr>");
		}
		out.print("</table>");
	
		con.close();
		
	
			
		
	
	}catch (Exception ex) {
		out.print(ex);
	}
%>
<form method="post" action="updateDeloffergiven.jsp">
 Input the ride offer you want to delete <br/> 
        
 
 Passenger: <input type = "text" name = "Cpassenger" > <br/>
 <br/> Date: <input type = "text" name = "Cdate" > <br/> 
 Time: <input type ="text" name="Ctime" > <br/> 
 
 


 
 
 
 
<input type= "submit" value = "Submit">
</form>

</body>
</html>